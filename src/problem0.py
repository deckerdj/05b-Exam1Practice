...
def problem0c(circle, n, window):
    ...
    # -------------------------------------------------------------------------
    givencircle = circle
    givencircle.attach_to(window)
    window.render(.5)
    radius = givencircle.radius
    center = givencircle.center
    for k in range(n):
        xvalue = givencircle.center.x + ((k + 1) * (2 * radius))
        newcircle = rg.Circle(rg.Point(xvalue, center.y), radius)
